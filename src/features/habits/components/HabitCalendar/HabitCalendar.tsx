...
import { format, subDays, addDays, isToday, isAfter, isSameDay, startOfMonth, endOfMonth, eachDayOfInterval } from 'date-fns';
...
const DateCell = styled.div<{
  ...
  cursor: ${props => (props.$isDue && !props.$isFuture) ? 'pointer' : 'default'};
  opacity: ${props => (props.$isDue && !props.$isFuture) ? 1 : 0.5};
  background-color: ${props => {
    if (props.$isToday) return props.$isCompleted ? props.theme.colors.successLight : props.theme.colors.backgroundAlt;
  ...
  color: ${props => props.$isCompleted ? props.theme.colors.success : props.theme.colors.text};
  font-weight: ${props => props.$isToday ? 'bold' : 'normal'};
  
  &:hover {
    background-color: ${props => 
      (props.$isDue && !props.$isFuture) ? 
        props.$isCompleted ? props.theme.colors.success : props.theme.colors.primaryLight 
        : props.$isCompleted ? props.theme.colors.successLight : props.theme.colors.background
    };
  }
`;
